...
| Running user TS     | Sucrase transpile (TS + import transforms) → evaluate as CJS module                                                                      | Full `tsc` in-browser is ~8 MB; Sucrase is fast and small. Type _checking_ is not required to judge solutions — tests judge behavior.                                                                                |
