...
describe('async-arrays-and-promises category', () => {
  it('satisfies the structural invariants', () => {
    expect(structuralIssues(asyncArraysAndPromises)).toEqual([]);
  });

  it.each(asyncArraysAndPromises.challenges.map((challenge) => [challenge.id, challenge] as const))(
