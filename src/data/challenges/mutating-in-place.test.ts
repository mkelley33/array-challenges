...
describe('mutating-in-place category', () => {
  it('satisfies the structural invariants', () => {
    expect(structuralIssues(mutatingInPlace)).toEqual([]);
  });

  it.each(mutatingInPlace.challenges.map((challenge) => [challenge.id, challenge] as const))(
