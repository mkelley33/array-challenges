...
describe('sets-and-set-algebra category', () => {
  it('satisfies the structural invariants', () => {
    expect(structuralIssues(setsAndSetAlgebra)).toEqual([]);
  });

  it.each(setsAndSetAlgebra.challenges.map((challenge) => [challenge.id, challenge] as const))(
