...
describe('access-and-search category', () => {
  it('satisfies the structural invariants', () => {
    expect(structuralIssues(accessAndSearch)).toEqual([]);
  });

  it.each(accessAndSearch.challenges.map((challenge) => [challenge.id, challenge] as const))(
