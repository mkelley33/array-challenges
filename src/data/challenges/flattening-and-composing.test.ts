...
describe('flattening-and-composing category', () => {
  it('satisfies the structural invariants', () => {
    expect(structuralIssues(flatteningAndComposing)).toEqual([]);
  });

  it.each(flatteningAndComposing.challenges.map((challenge) => [challenge.id, challenge] as const))(
