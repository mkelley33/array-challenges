...
describe('reduce-and-folding category', () => {
  it('satisfies the structural invariants', () => {
    expect(structuralIssues(reduceAndFolding)).toEqual([]);
  });

  it.each(reduceAndFolding.challenges.map((challenge) => [challenge.id, challenge] as const))(
