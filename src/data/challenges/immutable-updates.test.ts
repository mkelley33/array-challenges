...
describe('immutable-updates category', () => {
  it('satisfies the structural invariants', () => {
    expect(structuralIssues(immutableUpdates)).toEqual([]);
  });

  it.each(immutableUpdates.challenges.map((challenge) => [challenge.id, challenge] as const))(
